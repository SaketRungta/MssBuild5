	...
	StartSessionCompleteDelegate(FOnStartSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnStartSessionCompleteCallback))
{
	...
	if (!OnlineSubsystem)
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::USitSubsystem No Online Subsystem detected! Ensure a valid subsystem (e.g., Steam) is enabled in DefaultEngine.ini."));
		return;
	}
	...
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::USitSubsystem Online Subsystem does not support sessions! Check subsystem configuration."));
	}
}

#pragma region Session Operations

void USitSubsystem::CreateSession(const FCustomSessionSettings& InCustomSessionSettings)
{
	UE_LOG(MultiplayerSessionSubsystemLog, Log, TEXT("USitSubsystem::CreateSession Called"));

	if (!SessionInterface.IsValid())
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::CreateSession SessionInterface is INVALID. Check Online Subsystem.")); 
	...
	{		
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::CreateSession NAME_GameSession already exists, destroying it before creating a new one"));
...
#pragma endregion Session Settings
	...
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::CreateSession Failed to execute create session"));
...
void USitSubsystem::FindSessions()
{
	UE_LOG(MultiplayerSessionSubsystemLog, Log, TEXT("USitSubsystem::FindSessions Called"));
	
	if (!SessionInterface.IsValid())
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::FindSessions SessionInterface is INVALID. Check Online Subsystem."));
	...
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::FindSessions failed to execute find sessions"));
...
void USitSubsystem::JoinSessions(FOnlineSessionSearchResult& InSessionToJoin)
{
	UE_LOG(MultiplayerSessionSubsystemLog, Log, TEXT("USitSubsystem::JoinSessions Called"));
	
	if (!SessionInterface.IsValid())
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::JoinSessions SessionInterface is INVALID. Check Online Subsystem."));
	...
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::JoinSessions failed to execute join sessions"));
...
void USitSubsystem::DestroySession()
{
	UE_LOG(MultiplayerSessionSubsystemLog, Log, TEXT("USitSubsystem::DestroySession Called"));
	
	if (!SessionInterface.IsValid())
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::DestroySession SessionInterface is INVALID. Check Online Subsystem."));
	...
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::DestroySession failed to execute destroy sessions"));
...
void USitSubsystem::StartSession()
{
}

#pragma endregion Session Operations

FString USitSubsystem::GenerateSessionUniqueCode() const
{
	...
	UE_LOG(MultiplayerSessionSubsystemLog, Log, TEXT("USitSubsystem::GenerateSessionUniqueCode unique code for the currently created session '%s'"), *SumOfCurrentTime);

	/** Appends the 3 random alphabets and summation of current time to create unique code for this session */
	return SumOfCurrentTime;
}

#pragma region Session Operations On Completion Delegates Callbacks
	
void USitSubsystem::OnCreateSessionCompleteCallback(FName SessionName, bool bWasSuccessful)
{
	UE_LOG(MultiplayerSessionSubsystemLog, Log, TEXT("USitSubsystem::OnCreateSessionComplete created a session bWasSuccessful::%s"), bWasSuccessful ? TEXT("success") : TEXT("failed"));
...
void USitSubsystem::OnFindSessionsCompleteCallback(bool bWasSuccessful)
{
	UE_LOG(MultiplayerSessionSubsystemLog, Log, TEXT("USitSubsystem::OnFindSessionsCompleteCallback Completed finding sessions bWasSuccessful::%s"), bWasSuccessful ? TEXT("success") : TEXT("failed"));
	...
	{
		UE_LOG(MultiplayerSessionSubsystemLog, Error, TEXT("USitSubsystem::OnFindSessionsCompleteCallback search result array is empty 0 active sessions found"));
...
void USitSubsystem::OnJoinSessionCompleteCallback(FName SessionName,	EOnJoinSessionCompleteResult::Type Result)
{
	UE_LOG(MultiplayerSessionSubsystemLog, Log, TEXT("USitSubsystem::OnJoinSessionComplete Completed joining session"));
...
void USitSubsystem::OnDestroySessionCompleteCallback(FName SessionName, bool bWasSuccessful)
{
	UE_LOG(MultiplayerSessionSubsystemLog, Log, TEXT("USitSubsystem::OnJoinSessionComplete Completed destroying session bWasSuccessful::%s"), bWasSuccessful ? TEXT("success") : TEXT("failed"));
...
void USitSubsystem::OnStartSessionCompleteCallback(FName SessionName, bool bWasSuccessful)
{
}
